...
class ConvertHtmlToPdfExecutor implements Executor {
    ...
    private MessageResponse convertHtmlToPdf(String fileLocation, String filename, String htmlContent) {
        ...
        file.delete();
        boolean isUploaded =
            s3Client.uploadFileS3(fileLocation, filename + HelperConstants.FILE_EX_PDF,
        ...
        return new MessageResponse.Builder()
            .setHeader(HelperConstants.LOCATION,
                configRegistry.getReportCdnUrl() + filename + HelperConstants.FILE_EX_PDF).setContentTypeJson()
    ...
    private void convertHtmlToPdf(String srcFileName, String destFileName, long delayInMillsec) {
        try {
            String resizeCommand =
                new String("/usr/bin/wkhtmltopdf@" + srcFileName + "@--redirect-delay@" + delayInMillsec + "@"
                    + destFileName);
