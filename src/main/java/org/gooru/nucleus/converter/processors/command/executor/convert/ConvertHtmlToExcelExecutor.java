...
class ConvertHtmlToExcelExecutor implements Executor {
    ...
    private MessageResponse convertHtmltoExcel(String fileLocation, String filename, String htmlContent) {
        ...
        if (filename == null || (filename != null && filename.isEmpty())) {
            ...
            filename =
                InternalHelper.replaceSpecialCharWithUnderscore(filename) + HelperConstants.HYPEN
        ...
        if (file == null) {
            ServerValidatorUtility.throwASInternalServerError(ServerValidatorUtility
                .generateMessage(MessageCodeConstants.CVT004));
        }
        boolean isUploaded =
            s3Client.uploadFileS3(fileLocation, filename + HelperConstants.FILE_EX_XLSX,
                ConfigConstants.S3_REPORT_BUCKET_NAME);
        if (!isUploaded) {
            ServerValidatorUtility.throwASInternalServerError(ServerValidatorUtility
                .generateMessage(MessageCodeConstants.CVT005));
        }
        return new MessageResponse.Builder()
            .setHeader(HelperConstants.LOCATION,
                configRegistry.getReportCdnUrl() + filename + HelperConstants.FILE_EX_XLSX).setContentTypeJson()
                ...
                for (Element row : table.select(HelperConstants.TR)) {
                    ...
                    for (Element element : tds) {
                        ...
                        if (bgStyle != null && !bgStyle.isEmpty() && bgStyle.contains("#")) {
                            cell.setCellStyle(getCellStyle(workbook.createCellStyle(),
                                bgStyle.replaceAll(HelperConstants.BACKGROUND_COLOR, "")));
                        }
                        cell.setCellValue(element.text());
                        count++;
                    }
    ...
    private XSSFCellStyle getCellStyle(XSSFCellStyle cellStyle, String hexa) {
        Color c =
            new Color(Integer.valueOf(hexa.substring(1, 3), 16), Integer.valueOf(hexa.substring(3, 5), 16),
